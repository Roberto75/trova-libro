<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Parametri -->
	<xsl:param name="menuSelected">-1</xsl:param>
	...
	<xsl:template match="MENU">
		<xsl:choose>
			<xsl:when test="value!=''">
				<!-- se l'utente e' registrato non faccio visualizzzare il meni con Registrati -->
				<xsl:if test="not(($roles='Authenticated' or  $roles='Admin')  and  @utente = 'Registrato')">
					<xsl:element name="li">
						...
						<p>
							<xsl:element name="a">
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										<!-- Rimanda all'Url -->
										<xsl:attribute name="href"><xsl:value-of select="$contesto"/>/<xsl:value-of select="href"/>?menuTopSelected=<xsl:value-of select="position()"/></xsl:attribute>
									</xsl:otherwise>
								</xsl:choose>
								<xsl:value-of select="value"/>
							</xsl:element>
