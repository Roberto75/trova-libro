                          ...
                          <xsl:attribute name="href">
                            ...
                            <xsl:choose>
                              <xsl:when test="contains(href, '?')">
                                <xsl:value-of select="$contesto"/><xsl:value-of select="href"/>&amp;menuTopSelected=<xsl:value-of select="$menuTopSelected"/>&amp;menuLeftSelected=<xsl:value-of select="position()"/>
                              </xsl:when>
                              <xsl:otherwise>
                                <xsl:value-of select="$contesto"/><xsl:value-of select="href"/>?menuTopSelected=<xsl:value-of select="$menuTopSelected"/>&amp;menuLeftSelected=<xsl:value-of select="position()"/>
                              </xsl:otherwise>
